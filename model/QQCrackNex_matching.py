...
class CrackNex(nn.Module):
    def __init__(self, backbone):
        ...
        # -----------------------------------------------------------------
        # 1) QRetinex for decomposition (freeze its weights)
        # -----------------------------------------------------------------
        self.QRetinex = load_pretrained_model(
        ...
        # -------------------------------------------------
        # 2) Create two ResNet backbones (both 11-ch input):
        #    - rgb_backbone => 11-ch input
        #    - ref_backbone => 11-ch input
        #
        #    We will combine: (3-ch original) + (4-ch reflectance)
        #    + (4-ch illumination) = 11 channels total.
        # -------------------------------------------------
        rgb_backbone = resnet.__dict__[backbone](pretrained=True)
        ref_backbone = resnet.__dict__[backbone](pretrained=True)

        # ----------------------------------------
        # Convert rgb_backbone to accept 11-ch
        # ----------------------------------------
        # By default, many ResNets in PyTorch have
        #   rgb_backbone.conv1 = nn.Sequential(
        #       [Conv2d(3,64, kernel_size=3, stride=2, ...), ... ]
        #   )
        # or simply rgb_backbone.conv1 = Conv2d(3,64,3,2,3)
        #
        # If your code is storing them in .conv1[0], then do the same below.
        # Otherwise (if it's just .conv1), adapt accordingly.
        #
        old_weights_rgb = rgb_backbone.conv1[0].weight.clone()  # shape: (64, 3, 3, 3)
        new_conv_rgb = nn.Conv2d(
            11, 64, kernel_size=3, stride=2, padding=3, bias=False
        )
        with torch.no_grad():
            # Copy over the first 3-ch weights from pretrained
            new_conv_rgb.weight[:, :3, :, :] = old_weights_rgb
            # Zero out the remaining 8 channels
            new_conv_rgb.weight[:, 3:, :, :] = 0.0

        rgb_backbone.conv1[0] = new_conv_rgb  # replace old conv

        # Freeze entire rgb_backbone except the newly reinitialized conv1
        for name, param in rgb_backbone.named_parameters():
            param.requires_grad = False
        # Unfreeze the entire conv1[0] so the new slices can learn
        for param in rgb_backbone.conv1[0].parameters():
            param.requires_grad = True

        # ----------------------------------------
        # Convert ref_backbone to accept 11-ch
        # ----------------------------------------
        old_weights_ref = ref_backbone.conv1[0].weight.clone()  # shape: (64, 3, 3, 3)
        new_conv_ref = nn.Conv2d(
            11, 64, kernel_size=3, stride=2, padding=3, bias=False
        )
        with torch.no_grad():
            # Copy over the first 3-ch weights from pretrained
            new_conv_ref.weight[:, :3, :, :] = old_weights_ref
            # Zero out the remaining 8 channels
            new_conv_ref.weight[:, 3:, :, :] = 0.0

        ref_backbone.conv1[0] = new_conv_ref  # replace old conv

        # Freeze entire ref_backbone except the newly reinitialized conv1
        ...
        # ----------------------------------------
        # Reuse layers from the adjusted backbones
        # ----------------------------------------
        ...
        # -----------------------------------------------------------------
        # Example of "channel-split trick" from original code for rgb_layer2
        # (Leave this unchanged if you still want that behavior)
        # -----------------------------------------------------------------
        weight = self.rgb_layer2._modules['0'].conv1.weight.clone()
        self.rgb_layer2._modules['0'].conv1 = nn.Conv2d(
        ...
        self.rgb_layer2._modules['0'].downsample[0] = nn.Conv2d(
    ...
    def forward(self, img_s_list, hiseq_s_list, mask_s_list, img_q, hiseq_q, mask_q):
        """
        We now feed 11 channels => (3-ch original + 4-ch reflectance + 4-ch illumination)
        
        The “hiseq_q” images also need to be turned into 11 channels, 
        otherwise the new conv1 expecting 11 channels will fail.
        """
        h, w = img_q.shape[-2:]

        # ---------------------------
        # QRetinex decomposition for query
        # ---------------------------
        with torch.no_grad():
            reflectance_q, illumination_q = self.QRetinex(img_q)
        query_11 = torch.cat([img_q, reflectance_q, illumination_q], dim=1)

        # ---------------------------
        # QRetinex decomposition for hiseq_q
        # ---------------------------
        # If you want to handle hiseq the same way:
        with torch.no_grad():
            reflectance_q_hiseq, illumination_q_hiseq = self.QRetinex(hiseq_q)
        hiseq_q_11 = torch.cat([hiseq_q, reflectance_q_hiseq, illumination_q_hiseq], dim=1)

        # ---------------------------
        # Pass through "rgb" backbone
        # ---------------------------
        q_0 = self.rgb_layer0(query_11)
        q_0 = self.rgb_layer1(q_0)

        # Pass the 11-ch hiseq into the same layers
        q_0_hiseq = self.rgb_layer0(hiseq_q_11)
        q_0_hiseq = self.rgb_layer1(q_0_hiseq)

        # ---------------------------
        # Pass same 11-ch input to "ref" backbone
        # ---------------------------
        q_0_ref = self.ref_layer0(query_11)
        ...
        # ----------------------------------------------
        # Do the same for each support image
        # ----------------------------------------------
        feature_s_list = []
        feature_s_ref_list = []
        ...
        for k in range(len(img_s_list)):
            ...
            with torch.no_grad():
                reflectance_s, illumination_s = self.QRetinex(img_s_list[k])
            support_11 = torch.cat([img_s_list[k], reflectance_s, illumination_s], dim=1)

            # Decompose the support hiseq
            with torch.no_grad():
                reflectance_s_hiseq, illumination_s_hiseq = self.QRetinex(hiseq_s_list[k])
            hiseq_s_11 = torch.cat([hiseq_s_list[k], 
                                    reflectance_s_hiseq, 
                                    illumination_s_hiseq], dim=1)

            s_0 = self.rgb_layer0(support_11)
            s_0 = self.rgb_layer1(s_0)

            s_0_hiseq = self.rgb_layer0(hiseq_s_11)
            s_0_hiseq = self.rgb_layer1(s_0_hiseq)

            s_0_ref = self.ref_layer0(support_11)
        ...
        # ----------------------------------------------
        # Generate foreground/background prototypes, etc.
        ...
        for k in range(len(img_s_list)):
            # Original proto
            feature_fg = self.masked_average_pooling(
                feature_s_list[k], (mask_s_list[k] == 1).float()
            )[None, :]
            feature_bg = self.masked_average_pooling(
                feature_s_list[k], (mask_s_list[k] == 0).float()
            )[None, :]
            ...
            # Reflectance proto
            feature_ref_fg = self.masked_average_pooling(
                feature_s_ref_list[k], (mask_s_list[k] == 1).float()
            )[None, :]
            feature_ref_bg = self.masked_average_pooling(
                feature_s_ref_list[k], (mask_s_list[k] == 0).float()
            )[None, :]
            ...
            if self.training:
                # For training visualization or loss
                ...
                supp_out = torch.cat(
                    (supp_similarity_bg[:, None, ...], supp_similarity_fg[:, None, ...]),
                    dim=1
                ) * 10.0
                ...
                supp_out_ref = torch.cat(
                    (supp_similarity_ref_bg[:, None, ...], supp_similarity_ref_fg[:, None, ...]),
                    dim=1
                ) * 10.0
        ...
        # Average K foreground prototypes / K background prototypes
        ...
        if self.training:
            ...
            self_out = torch.cat(
                (self_similarity_bg[:, None, ...], self_similarity_fg[:, None, ...]),
                dim=1
            ) * 10.0
    ...
    def similarity_func(self, feature_q, fg_proto, bg_proto):
        ...
        out = torch.cat(
            (similarity_bg[:, None, ...], similarity_fg[:, None, ...]),
            dim=1
        ) * 10.0
        return out
